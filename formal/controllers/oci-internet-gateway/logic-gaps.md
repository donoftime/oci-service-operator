...
- `UpdateInternetGateway` now reconciles defined tags in addition to display name and freeform tags.
- Managed internet gateway reconciles now continue using `status.ocid` after create/bind, so supported drift updates do not fall back to create-by-name behavior.
- `spec.compartmentId` drift is now reconciled in place through OCI's internet-gateway compartment-move API before other supported updates are applied.
