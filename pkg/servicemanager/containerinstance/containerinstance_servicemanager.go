...
func (c *ContainerInstanceServiceManager) CreateOrUpdate(ctx context.Context, obj runtime.Object, req ctrl.Request) (servicemanager.OSOKResponse, error) {
	...
	if strings.TrimSpace(string(ci.Spec.ContainerInstanceId)) == "" {
		...
		if ciOcid == nil {
			...
			c.Log.InfoLog(fmt.Sprintf("ContainerInstance %s is Provisioning", ci.Spec.DisplayName))
			ci.Status.OsokStatus = util.UpdateOSOKStatusCondition(ci.Status.OsokStatus,
		...
		ci.Status.OsokStatus = util.UpdateOSOKStatusCondition(ci.Status.OsokStatus,
			...
			fmt.Sprintf("ContainerInstance %s is %s", *ciInstance.DisplayName, ciInstance.LifecycleState), c.Log)
		c.Log.InfoLog(fmt.Sprintf("ContainerInstance %s is %s", *ciInstance.DisplayName, ciInstance.LifecycleState))
		...
		// Only displayName and tags can be updated
		...
		c.Log.InfoLog(fmt.Sprintf("ContainerInstance %s is bound/updated", *ciInstance.DisplayName))
	}
	...
	if ciInstance.LifecycleState == containerinstances.ContainerInstanceLifecycleStateFailed {
		ci.Status.OsokStatus = util.UpdateOSOKStatusCondition(ci.Status.OsokStatus,
			...
			fmt.Sprintf("ContainerInstance %s creation Failed", *ciInstance.DisplayName), c.Log)
		c.Log.InfoLog(fmt.Sprintf("ContainerInstance %s creation Failed", *ciInstance.DisplayName))
